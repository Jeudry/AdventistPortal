...
class PushNotificationService(
  ...
  fun registerDevice(
    ...
    // Solo validar token si Firebase está disponible
    if(existing == null && firebasePushNotificationService != null) {
      if(!firebasePushNotificationService.isValidToken(trimmedToken)) {
  ...
  fun sendWithRetry(
    ...
    // Si Firebase no está disponible, solo logear y retornar
    if (firebasePushNotificationService == null) {
      logger.warn("Firebase is disabled. Skipping push notification to ${notification.recipients.size} devices.")
      return
    }
