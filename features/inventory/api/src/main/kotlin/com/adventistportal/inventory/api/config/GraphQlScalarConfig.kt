...
/**
 * Backs the `scalar BigDecimal`, `scalar DateTime` and `scalar UUID` declarations in
 * schema.graphqls and quotes.graphqls. graphql-java resolves scalars by name and refuses to build the
...
class GraphQlScalarConfig {
    ...
    private fun dateTimeScalar(): GraphQLScalarType =
        scalar("DateTime", "An instant in time, as an ISO-8601 string in UTC.") { input ->
            when (input) {
                is Instant -> input
