...
create table user_service.email_verification_tokens (id bigint generated by default as identity, created_at timestamp(6) with time zone not null, expires_at timestamp(6) with time zone not null, token varchar(255) not null unique, used_at timestamp(6) with time zone, user_id uuid not null, primary key (id));
