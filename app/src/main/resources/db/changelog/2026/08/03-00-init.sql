...
--comment: Complete initial schema, generated from the JPA model. Supersedes the Flyway V1-V3 migrations and the partial 2025/12 changelogs.
...
create table chat_service.chat_participants_cross_ref (chat_id uuid not null, participant_id uuid not null, constraint idx_chat_participant_participant_id_chat_id primary key (chat_id, participant_id));
...
create table inventory_service.article_variant_dimensions (variant_id uuid not null, "dimensions_depthCm" float(53), "dimensions_heightCm" float(53), "dimensions_label" varchar(255), "dimensions_weightKg" float(53), "dimensions_widthCm" float(53));
...
create table inventory_service.articles (id uuid not null, created_at timestamp(6) with time zone, description_template TEXT, is_active boolean not null, name_template varchar(255) not null, type varchar(255) not null check ((type in ('Rental','Sale'))), updated_at timestamp(6) with time zone, category_id uuid, primary key (id));
